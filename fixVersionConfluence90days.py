...
def fix_version_90day():
    ...
    confluence_page_id = Variable.get("CONFLUENCE_PAGE_ID_90DAY") #updated variable
    ...
    query = f'project = {jira_project_key} AND issuetype = Features AND fixVersion CHANGED AFTER startOfDay(-90d) AND key != PMZ-721' #updated query
    ...
    seven_days_ago = (datetime.now() - timedelta(days=90))
    ...
    start_at = 0
    max_results = 50  # Set the maximum number of results per page

    while True:
        # Fetch a page of issues
        issues = jira.search_issues(query, startAt=start_at, maxResults=max_results, fields="summary,status,fixVersions,created,components")

        if not issues:
            break  # No more issues to fetch, break out of the loop
    ...
    confluence.update_page(
        ...
        title=f"PMZ Fix Versions Changes - 90 Day Lookback",
        body=page_content,
        representation='wiki'
    )
...
def base_function():
    execute_fix_version_90day = PythonOperator(
        task_id='ExecuteCode',
        python_callable=fix_version_90day
    )

    execute_fix_version_90day
