...
def fix_version_1wk():
    ...
    confluence_page_id = Variable.get("CONFLUENCE_PAGE_ID_1WK") #updated variable
    ...
    query = f'project = {jira_project_key} AND issuetype = Features AND fixVersion CHANGED AFTER startOfDay(-7d) AND key != PMZ-721' #updated query
    ...
    confluence.update_page(
        ...
        title=f"Updated Issue Fix Versions Since Last Week",
        body=page_content,
        representation='wiki'
    )
...
def base_function():
    execute_fix_version_1wk = PythonOperator(
        task_id='ExecuteCode',
        python_callable=fix_version_1wk
    )

    execute_fix_version_1wk
