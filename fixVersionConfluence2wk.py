...
def fix_version_2wk():
    ...
    confluence_page_id = Variable.get("CONFLUENCE_PAGE_ID_2WK") #updated variable
    ...
    query = f'project = {jira_project_key} AND issuetype = Features AND fixVersion CHANGED AFTER startOfDay(-14d) AND key != PMZ-721' #updated query
    ...
    confluence.update_page(
        ...
        title=f"PMZ Fix Versions Changes - 2 Week Lookback",
        body=page_content,
        representation='wiki'
    )
...
def base_function():
    execute_fix_version_2wk = PythonOperator(
        task_id='ExecuteCode',
        python_callable=fix_version_2wk
    )

    execute_fix_version_2wk
